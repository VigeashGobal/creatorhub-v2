...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fallback to sample data for POC demo stability
    try {
      const body = await request.json().catch(() => ({} as any))
      const demo = buildSampleAnalytics(body?.name, body?.email)
      return NextResponse.json(demo)
    } catch {
      const demo = buildSampleAnalytics()
      return NextResponse.json(demo)
    }
  }
}
...
function buildSampleAnalytics(name: string = 'Demo User', email: string = 'demo@example.com') {
  return {
    user: { name, email },
    platforms: {
      youtube: {
